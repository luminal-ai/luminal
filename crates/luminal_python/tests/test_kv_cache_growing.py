...
def test_dynamic_kv_cache_graph_reuses_compiled_decode_across_cache_growth():
    """A single dynamic decode graph should run at larger cache lengths."""
    import copy

    from transformers import DynamicCache, LlamaConfig, LlamaForCausalLM

    from luminal.main import _detect_factory_capsule
    ...
    try:
        ...
        model = LlamaForCausalLM(config).eval()
        compiled = torch.compile(model, backend=inspector_backend, fullgraph=True)
        ...
        gm = copy.deepcopy(capture["gm"]).eval()
        gm, user_inputs, original_weights, _ = _reinternalize_lifted_params(
            gm, capture["example_inputs"]
        )
        ...
        assert strip_ok, "Expected SymInt placeholders to be rewritten to tensor sizes"
        assert dynamic_shapes is not None, "Expected a dynamic decode graph"

        ep = torch.export.export(
            gm, tuple(user_inputs), dynamic_shapes=dynamic_shapes, **_export_kwargs()
        )
        ep = ep.run_decompositions(_decomp_table())
        compiled_decode = _save_and_compile(
            ep,
            _detect_factory_capsule(user_inputs),
            1,
            original_weights=original_weights,
        )

        assert compiled_decode.has_dynamic_dims

        def grow_cache_inputs(inputs):
            grown = []
            for i, tensor in enumerate(inputs):
                if i == 0:
                    grown.append(tensor.clone())
                elif tensor.dim() >= 3:
                    grown.append(
                        torch.cat(
                            [tensor, tensor.narrow(2, tensor.shape[2] - 1, 1)], dim=2
                        )
                    )
                else:
                    grown.append(tensor.clone())
        ...
        input_sets = [
            user_inputs,
            grow_cache_inputs(user_inputs),
            grow_cache_inputs(grow_cache_inputs(user_inputs)),
        ]

        seen_cache_output_lens = []
        for inputs in input_sets:
            compiled_decode._graph.auto_set_dims_from_input_shapes(
                [list(t.shape) for t in inputs]
            )
            expected_shapes = compiled_decode._graph.resolve_output_shapes()
            outputs = compiled_decode(*inputs)
            actual_shapes = [list(t.shape) for t in outputs]
            assert actual_shapes == expected_shapes
            seen_cache_output_lens.append(actual_shapes[0][2])

        assert seen_cache_output_lens == sorted(seen_cache_output_lens)
        assert len(set(seen_cache_output_lens)) == len(seen_cache_output_lens)
