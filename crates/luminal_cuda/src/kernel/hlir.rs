...
pub struct ComplexElementwiseOp {
    ...
    /// boundary_remaps[k] sits between step k and step k+1 to remap the strides when fusing two elementwise ops.
...
(rule
    ...
    fn extract<'a>(
        ...
        (
            ...
            vec![], // sources come from IList walking in egglog_to_llir
        )
    }
}
...
impl KernelOp for ComplexElementwiseOp {
    fn compile(
        ...
        let vars = self
            ...
            .chain(self.out_stride.iter().flat_map(|e| e.dyn_vars()))
            .chain(
                self.boundary_remaps
                    .iter()
                    .flat_map(|(inner_out, outer_in)| {
                        inner_out
                            .iter()
                            .chain(outer_in.iter())
                            .flat_map(|e| e.dyn_vars())
        ...
        let z_per_step: Vec<Expression> = if n_boundaries == 0 {
            ...
            for k in (0..n_boundaries).rev() {
                ...
                let z_inner = if inner_out == outer_in {
                    // no remapping if strides are equal
                    z_outer
                } else if *inner_out == rm {
                    // if inner_out is row-major, then its offset == z_inner
                    let m_template = flatten_mul_strides(&self.out_shape, outer_in);
                    m_template.substitute('z', z_outer).simplify()
                } else {
                    // in general, we do z_outer -> offset -> z_inner
                    let m_template = flatten_mul_strides(&self.out_shape, outer_in);
                    let m = m_template.substitute('z', z_outer).simplify();
                    let inv_template = inverse_flatten_mul_strides(&self.out_shape, inner_out);
...
{dyn_defines}
extern \"C\" {{
    __global__ void complex_elementwise_k({dtype} *out{input_params}{dyn_dims_param}) {{
        long long const_z = (long long)blockIdx.x * blockDim.x + threadIdx.x;
        {body}
    }}
}}"
