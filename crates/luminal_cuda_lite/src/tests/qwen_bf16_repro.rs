...
/// moe() with intermediate outputs for debugging: returns
/// (routing_weights, top_k_indices, top_k_values, gate_up_out, down_out, out).
fn moe_debug(
    x: GraphTensor,
    router: GraphTensor,
    gate_up_weights: GraphTensor,
    ...
    let mut weights_exp = top_k_values.unsqueeze(top_k_values.dims().len());
    weights_exp.shape.expand(down_out.dims());
    let out = (down_out * weights_exp).sum(n - 1);
    (
        routing_weights,
        top_k_indices,
        top_k_values,
...
#[test]
#[ignore = "debug instrument for candidate hunts; run on demand"]
fn mini_qwen_stage5_dump_bad_genome() {
    // Custom fuzz loop that, on disagreement, prints the genome's kernel
    // composition and a window of wrong values — pinpoints the bad candidate.
    ...
    // CPU ground truth: stage 4's x_mlp is fuzz-validated; compute the MoE
    // from it in plain Rust.
    let x_mlp = {
        let mut m4 = build_mini_qwen_stage(DType::F32, 4);
        m4.cx
            .build_search_space::<CudaRuntime>(CompileOptions::default());
        let mut rt = CudaRuntime::initialize(stream.clone());
        set_inputs(&m4, &d, DType::F32, &mut rt);
        rt = m4.cx.search(rt, CompileOptions::default().search_graph_limit(1));
        set_inputs(&m4, &d, DType::F32, &mut rt);
        rt.execute(&m4.cx.dyn_map);
        rt.get_f32(m4.out.id)[..S * H].to_vec()
    };
    let mut cpu_moe = vec![0.0f32; S * H];
    for s in 0..S {
        let xr = &x_mlp[s * H..(s + 1) * H];
        let mut logits = [0.0f32; NUM_EXPERTS];
        for (e, logit) in logits.iter_mut().enumerate() {
            *logit = (0..H).map(|i| xr[i] * d.router_w[e * H + i]).sum();
        }
        ...
        let top: Vec<usize> = order[..TOP_K].to_vec();
        let wsum: f32 = top.iter().map(|e| probs[*e]).sum();
        for &e in &top {
            let w_slot = probs[e] / wsum;
            let mut hidden = [0.0f32; MOE_I];
            for o in 0..MOE_I {
                let g: f32 = (0..H)
                    .map(|i| xr[i] * d.gate_up_w[(e * 2 * MOE_I + o) * H + i].to_f32())
                    .sum();
                let u: f32 = (0..H)
                    .map(|i| xr[i] * d.gate_up_w[(e * 2 * MOE_I + MOE_I + o) * H + i].to_f32())
                    .sum();
                hidden[o] = (g / (1.0 + (-g).exp())) * u;
            }
            for hcol in 0..H {
                let dv: f32 = (0..MOE_I)
                    .map(|i| hidden[i] * d.down_w[(e * H + hcol) * MOE_I + i].to_f32())
                    .sum();
                cpu_moe[s * H + hcol] += w_slot * dv;
            }
        }
    }
    let ref_vs_cpu = reference
        .iter()
        .zip(&cpu_moe)
        .map(|(a, b)| (a - b).abs())
        .fold(0.0f32, f32::max);
    println!("reference vs CPU ground truth: max_diff={ref_vs_cpu}");

    let egraph = m.cx.egraph().unwrap();
    let ops = m.cx.egglog_ops().unwrap();
    let mut rng = StdRng::seed_from_u64(0x91 + 7777);
    let mut prev = luminal::prelude::FxHashSet::default();
    let initial = random_initial_choice(egraph, &mut rng);
    prev.insert(hash_choice_set(&initial));
    let mut base = initial;
    let mut tested = 0;
    let mut bad = 0;
    'outer: for _ in 0..100 {
        let offspring = extract_generation(egraph, &base, 10, 2, &mut prev, &mut rng);
        ...
        for genome in offspring {
            ...
            luminal::prelude::unroll_loops_in_llir(&mut llir);
            let mut rt = CudaRuntime::initialize(stream.clone());
            rt.load_llir(&llir);
            set_inputs(&m, &d, DType::F32, &mut rt);
            rt.execute(&m.cx.dyn_map);
            let result = rt.get_f32(m.out.id);
            ...
            if max_diff > 1e-2 {
                ...
                let vs_cpu = cpu_moe
                    ...
                    .map(|(a, b)| (a - b).abs())
                    .fold(0.0f32, f32::max);
                println!("  genome vs CPU truth: max_diff={vs_cpu}");
                let mut names = std::collections::BTreeMap::<String, usize>::new();
                for node in llir.node_indices() {
                    if let Some(k) = llir[node].to_dialect::<dyn crate::kernel::KernelOp>() {
                        *names.entry(k.kernel_name().to_string()).or_default() += 1;
                    }
                }
                println!("BAD genome #{tested} max_diff={max_diff}: {names:?}");
                for node in llir.node_indices() {
                    let dbg = format!("{:?}", llir[node]);
                    if dbg.contains("KernelMoEGemv") || dbg.contains("GenericMatmul") {
                        println!("  op: {}", &dbg[..dbg.len().min(360)]);
                    }
                }
                let diffs: Vec<usize> = (0..n_out)
                    .filter(|i| (reference[*i] - result[*i]).abs() > 1e-2)
                    .take(8)
                    .collect();
                println!(
                    "  first wrong idxs: {diffs:?} (ref {:?} got {:?})",
                    diffs.iter().map(|i| reference[*i]).collect::<Vec<_>>(),
                    diffs.iter().map(|i| result[*i]).collect::<Vec<_>>()
                );
                if bad >= 1 {
                    break 'outer;
                }
    ...
    println!("tested {tested} genomes, {bad} bad");
    assert_eq!(bad, 0, "found {bad} disagreeing genomes");
}

#[test]
#[ignore = "debug instrument for candidate hunts; run on demand"]
fn mini_qwen_stage5_inspect_moe_gemv_enodes() {
    // Enumerate every KernelMoEGemv enode and the other op heads sharing its
    // eclass. A mis-union shows up as an eclass containing unexpected
    // members (anything other than the two expert matmul Sums).
    ...
    let mut found = 0;
    for (id, (head, children)) in egraph.enodes.iter() {
        if head != "Op" || children.is_empty() {
            continue;
        }
        let kinds = kinds_of(&children[0]);
        if !kinds.iter().any(|k| k.contains("KernelMoEGemv")) {
            continue;
        }
        found += 1;
        let class = &egraph.node_to_class[id];
        let mut sibling_kinds: Vec<String> = Vec::new();
        if let Some((_l, ns)) = egraph.eclasses.get(class) {
            for n in ns {
                let (h, ch) = &egraph.enodes[n];
                if h == "Op" && !ch.is_empty() {
                    sibling_kinds.extend(kinds_of(&ch[0]));
                } else {
                    sibling_kinds.push(h.clone());
                }
            }
        }
        println!("MoEGemv Op #{found} in class {class:?}: siblings {sibling_kinds:?}");
    }
    println!("total MoEGemv Op applications: {found}");
}

#[test]
#[ignore = "debug instrument for candidate hunts; run on demand"]
fn mini_qwen_stage5_localize_bad_intermediate() {
    // Standalone MoE-with-debug-outputs graph fed the (fuzz-validated)
    // stage-4 x_mlp as a direct input — then fuzz genomes and, on a wrong
    // final output, print which intermediate diverges first.
    use luminal::egglog_utils::{
        egglog_to_llir, extract_generation, hash_choice_set, random_initial_choice,
        validate_choice_set,
    };
    ...
    let d = mini_data();
    // x_mlp from stage 4. Default extractions occasionally hit the known
    // materialization corner case; retry until one loads.
    let x_mlp = (0..5)
        .find_map(|_| {
        ...
        .expect("stage 4 reference failed 5 times");
    ...
    let gate_up_w = cx.tensor((NUM_EXPERTS, 2 * MOE_I, H)).as_dtype(DType::Bf16);
    let down_w = cx.tensor((NUM_EXPERTS, H, MOE_I)).as_dtype(DType::Bf16);
    let (routing, topk_i, topk_v, gu_out, dn_out, out) = moe_debug(x, router_w, gate_up_w, down_w);
    let routing = routing.output();
    let topk_i = topk_i.output();
    let topk_v = topk_v.output();
    let gu_out = gu_out.output();
    let dn_out = dn_out.output();
    let out = out.output();
    cx.build_search_space::<CudaRuntime>(CompileOptions::default());

    let set = |rt: &mut CudaRuntime| {
        rt.set_data(x, x_mlp.clone());
        rt.set_data(router_w, d.router_w.clone());
        rt.set_data(gate_up_w, d.gate_up_w.clone());
        rt.set_data(down_w, d.down_w.clone());
    };
    let rt = (0..5)
        .find_map(|_| {
            std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                let mut rt = CudaRuntime::initialize(stream.clone());
                set(&mut rt);
                let mut rt = cx.search(rt, CompileOptions::default().search_graph_limit(1));
                set(&mut rt);
                rt.execute(&cx.dyn_map);
                rt
            }))
        ...
        .expect("moe reference failed 5 times");
    let r_routing = rt.get_f32(routing.id)[..S * NUM_EXPERTS].to_vec();
    let r_topk_i = rt.get_i32(topk_i.id)[..S * TOP_K].to_vec();
    let r_topk_v = rt.get_f32(topk_v.id)[..S * TOP_K].to_vec();
    let r_gu = rt.get_f32(gu_out.id)[..S * TOP_K * 2 * MOE_I].to_vec();
    let r_dn = rt.get_f32(dn_out.id)[..S * TOP_K * H].to_vec();
    let r_out = rt.get_f32(out.id)[..S * H].to_vec();
    drop(rt);
    println!("ref topk_i: {r_topk_i:?}");
    println!("ref topk_v: {r_topk_v:?}");
    // CPU expected gate_up first element of each (s,k) slot.
    let cpu_gu = |s: usize, k: usize, o: usize| -> f32 {
        let e = r_topk_i[s * TOP_K + k] as usize;
        (0..H)
            .map(|i| x_mlp[s * H + i] * d.gate_up_w[(e * 2 * MOE_I + o) * H + i].to_f32())
            .sum()
    };
    for e in 0..NUM_EXPERTS {
        let v: f32 = (0..H)
            .map(|i| x_mlp[H + i] * d.gate_up_w[(e * 2 * MOE_I) * H + i].to_f32())
            .sum();
        println!("row1 expert {e}: gate_up dot={v:.6}");
    }
    // CPU hidden for slot 2 (s=1,k=0) from the reference gate_up, then the
    // down dot for every expert — identifies which expert a wrong down value
    // corresponds to.
    {
        let slot = 2;
        let gu = &r_gu[slot * 2 * MOE_I..(slot + 1) * 2 * MOE_I];
        let hidden: Vec<f32> = (0..MOE_I)
            .map(|o| {
                let g = gu[o];
                let u = gu[MOE_I + o];
                (g / (1.0 + (-g).exp())) * u
        ...
        for e in 0..NUM_EXPERTS {
            let v: f32 = (0..MOE_I)
                .map(|i| hidden[i] * d.down_w[(e * H) * MOE_I + i].to_f32())
                .sum();
            println!("slot2 down expert {e}: dot={v:.6}");
        }
    }
    for s in 0..S {
        for k in 0..TOP_K {
            let slot = s * TOP_K + k;
            println!(
                "slot {slot} (s={s},k={k},e={}): cpu={:.6} ref={:.6}",
                r_topk_i[slot],
                cpu_gu(s, k, 0),
                r_gu[slot * 2 * MOE_I]
            );
    ...
    'outer: for _ in 0..200 {
        ...
        for genome in offspring {
            ...
            luminal::prelude::unroll_loops_in_llir(&mut llir);
            // Some genomes hit the known CUDA-graph materialization corner
            // case ("missing cached buffer"); skip those like the search does.
            ...
            let Ok(rt) = load_result else {
                continue;
            };
            let g_out = rt.get_f32(out.id);
            let max_diff = r_out
                .iter()
                .zip(&g_out[..S * H])
                .map(|(a, b)| (a - b).abs())
                .fold(0.0f32, f32::max);
            tested += 1;
            // Judge against pure-CPU truth for down slot 2 col 0: expert 3,
            // gate_up and hidden computed from x_mlp + weights directly.
            let g_dn_slot2 = rt.get_f32(dn_out.id)[2 * H];
            let cpu_dn_slot2: f32 = {
                let e = 3usize; // topk row 1, slot 0
                let xr = &x_mlp[H..2 * H];
                let hidden: Vec<f32> = (0..MOE_I)
                    .map(|o| {
                        let g: f32 = (0..H)
                        ...
                        let u: f32 = (0..H)
                            .map(|i| {
                                xr[i] * d.gate_up_w[(e * 2 * MOE_I + MOE_I + o) * H + i].to_f32()
                            })
                            .sum();
                        (g / (1.0 + (-g).exp())) * u
                    })
            ...
            let cpu_wrong = (g_dn_slot2 - cpu_dn_slot2).abs() > 1e-2;
            {
                use std::io::Write;
                let tag = if cpu_wrong { "CPUWRONG" } else { "CPUOK" };
                let path = format!("/tmp/genome_{tag}.txt");
                if !std::path::Path::new(&path).exists() {
                    let mut f = std::fs::File::create(&path).unwrap();
                    writeln!(f, "genome #{tested} down_slot2 got={g_dn_slot2} cpu={cpu_dn_slot2}").unwrap();
                    let order = luminal::prelude::petgraph::algo::toposort(&llir, None).unwrap();
                    for node in order {
                        let mut inputs: Vec<_> = llir
                            .edges_directed(node, luminal::prelude::petgraph::Direction::Incoming)
                            .map(|e| {
                        ...
                        inputs.sort();
                        let dbg = format!("{:?}", llir[node]);
                        writeln!(f, "{} <- {:?}: {}", node.index(), inputs, &dbg[..dbg.len().min(300)]).unwrap();
                    }
                    println!("wrote {path}");
                }
            }
            if max_diff > 1e-2 {
                bad += 1;
                let cmp_f32 = |name: &str, reference: &[f32], id: NodeIndex, n: usize, rt: &CudaRuntime| {
                    let got = rt.get_f32(id);
                    let md = reference
                        .iter()
                        .zip(&got[..n])
                        .map(|(a, b)| (a - b).abs())
                        .fold(0.0f32, f32::max);
                    println!("  {name}: max_diff={md}");
                };
                println!("BAD genome #{tested} out max_diff={max_diff}");
                cmp_f32("routing", &r_routing, routing.id, S * NUM_EXPERTS, &rt);
                let g_topk_i = rt.get_i32(topk_i.id);
                println!(
                    "  topk_i: ref {r_topk_i:?} got {:?}",
                    &g_topk_i[..S * TOP_K]
                );
                cmp_f32("topk_v", &r_topk_v, topk_v.id, S * TOP_K, &rt);
                cmp_f32("gate_up", &r_gu, gu_out.id, S * TOP_K * 2 * MOE_I, &rt);
                cmp_f32("down", &r_dn, dn_out.id, S * TOP_K * H, &rt);
                let g_dn = rt.get_f32(dn_out.id);
                let wrong_dn: Vec<usize> = (0..S * TOP_K * H)
                    .filter(|i| (r_dn[*i] - g_dn[*i]).abs() > 1e-2)
                    .collect();
                let dn_rows: std::collections::BTreeSet<usize> =
                    wrong_dn.iter().map(|i| i / H).collect();
                println!(
                    "  down wrong elems: {} of {} in (s,k) slots {dn_rows:?}; first idx {:?} ref {:?} got {:?}",
                    wrong_dn.len(),
                    S * TOP_K * H,
                    wrong_dn.first(),
                    wrong_dn.first().map(|i| r_dn[*i]),
                    wrong_dn.first().map(|i| g_dn[*i]),
                );
                let g_gu = rt.get_f32(gu_out.id);
                let wrong: Vec<usize> = (0..S * TOP_K * 2 * MOE_I)
                    .filter(|i| (r_gu[*i] - g_gu[*i]).abs() > 1e-2)
                    .collect();
                let rows: std::collections::BTreeSet<usize> =
                    wrong.iter().map(|i| i / (2 * MOE_I)).collect();
                println!(
                    "  gate_up wrong elems: {} of {} in (s,k) slots {rows:?}; first idx {:?} ref {:?} got {:?}",
                    wrong.len(),
                    S * TOP_K * 2 * MOE_I,
                    wrong.first(),
                    wrong.first().map(|i| r_gu[*i]),
                    wrong.first().map(|i| g_gu[*i]),
                );
                let g_dn = rt.get_f32(dn_out.id);
                let wrong_dn: Vec<usize> = (0..S * TOP_K * H)
                    .filter(|i| (r_dn[*i] - g_dn[*i]).abs() > 1e-2)
...
#[test]
#[ignore = "debug instrument: dump LLIR of failing topk genomes"]
fn debug_topk_genomes() {
    use crate::tests::utilities::summarize_llir;
    use luminal::egglog_utils::{
        egglog_to_llir, extract_generation, hash_choice_set, random_initial_choice,
        validate_choice_set,
    };
    use luminal::graph::unroll_loops_in_llir;
    use rand::{SeedableRng, rngs::StdRng};
    ...
    for _ in 0..30 {
        ...
        base = offspring.last().unwrap().clone();
        all.extend(offspring);
    }
    for genome in all {
        if validate_choice_set(egraph, &genome, ops).is_err() {
            continue;
        ...
        let summary = summarize_llir(&llir_graph);
        let fused = summary.contains("KernelStableSortIdx");
        let run = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            let mut rt = CudaRuntime::initialize(stream.clone());
            rt.load_llir(&llir_graph);
            rt.set_data(x, x_data.clone());
            rt.execute(&cx.dyn_map);
            rt.get_f32(out.id)[..12].to_vec()
        }));
        let Ok(got) = run else {
            println!("genome panicked (skipped), fused={fused}");
            continue;
        };
        tested += 1;
        let ok = got
        ...
        println!("genome fused={fused} ok={ok}");
        if !ok && shown_fail < 3 {
            shown_fail += 1;
            println!("  got: {got:?}");
            println!("--- LLIR ---\n{summary}\n---");
    ...
    println!("tested {tested} genomes, shown {shown_fail} failures");
}
