...
/// Exercise the optional Hopper adapter against the defining attention-sink
/// equation. The library path is deliberately opt-in: ordinary test hosts do
/// not need a separately packaged FlashAttention-3 shared object.
#[test]
fn external_fa3_sink_decode_matches_reference() {
    let Ok(library_path) = std::env::var("LUMINAL_EXTERNAL_FA3_LIBRARY") else {
        return;
    };
    if !crate::tests::utilities::gpu_supports_flashinfer() {
        return;
    ...
    let context_lens = [13usize, 7usize];
    let cache_slots = 31usize;
    let max_context_len = 32usize;
    let sm_scale = 1.0 / (HEAD_DIM as f32).sqrt();
    let q_token_major = round_to_bf16(&deterministic_f32(batch_size * HIDDEN, 0.017, 0.12));
    let k = round_to_bf16(&deterministic_f32(cache_slots * KV_DIM, 0.023, 0.11));
    let v = round_to_bf16(&deterministic_f32(cache_slots * KV_DIM, 0.031, 0.13));
    let sinks: Vec<f32> = (0..N_HEADS).map(|h| -0.35 + h as f32 * 0.07).collect();
    let token_indices: Vec<i32> = (0..context_lens[0])
        .map(|i| (i * 2) as i32)
        .chain((0..context_lens[1]).map(|i| (i * 2 + 1) as i32))
        .collect();
    let kv_indptr = vec![
        0i32,
        context_lens[0] as i32,
        (context_lens[0] + context_lens[1]) as i32,
    ];
    let qo_indptr = vec![0i32, 1, 2];

    // SinkAttention's graph boundary is heads-major; the external FA3 output
    // is token-major for direct consumption by the projection.
    let mut q_heads_major = vec![0.0f32; q_token_major.len()];
    for b in 0..batch_size {
        for h in 0..N_HEADS {
            for d in 0..HEAD_DIM {
                q_heads_major[h * batch_size * HEAD_DIM + b * HEAD_DIM + d] =
                    q_token_major[b * HIDDEN + h * HEAD_DIM + d];
            }
        }
    }

    let mut expected = vec![0.0f32; batch_size * HIDDEN];
    for b in 0..batch_size {
        let begin = kv_indptr[b] as usize;
        let end = kv_indptr[b + 1] as usize;
        for h in 0..N_HEADS {
            let kv_head = h / KV_GROUPS;
            let mut scores = Vec::with_capacity(end - begin);
            for &slot in &token_indices[begin..end] {
                let q_base = b * HIDDEN + h * HEAD_DIM;
                let k_base = slot as usize * KV_DIM + kv_head * HEAD_DIM;
                let score = (0..HEAD_DIM)
                    .map(|d| q_token_major[q_base + d] * k[k_base + d])
                    .sum::<f32>()
                    * sm_scale;
                scores.push(score);
            }
            let maximum = scores
                .iter()
                .copied()
                .fold(sinks[h], |maximum, score| maximum.max(score));
            let denominator = (sinks[h] - maximum).exp()
                + scores
                    .iter()
                    .map(|score| (*score - maximum).exp())
                    .sum::<f32>();
            for d in 0..HEAD_DIM {
                expected[b * HIDDEN + h * HEAD_DIM + d] = token_indices[begin..end]
                    .iter()
                    .zip(&scores)
                    .map(|(&slot, score)| {
                        let v_base = slot as usize * KV_DIM + kv_head * HEAD_DIM;
                        (*score - maximum).exp() / denominator * v[v_base + d]
                    })
                    .sum();
            }
        }
    ...
    let q_buf = copy_to_dev(&stream, &f32s_to_bf16_bytes(&q_heads_major));
    let k_buf = copy_to_dev(&stream, &f32s_to_bf16_bytes(&k));
    let v_buf = copy_to_dev(&stream, &f32s_to_bf16_bytes(&v));
    let token_indices_buf = copy_to_dev(&stream, &token_indices);
    let qo_indptr_buf = copy_to_dev(&stream, &qo_indptr);
    let kv_indptr_buf = copy_to_dev(&stream, &kv_indptr);
    let sinks_buf = copy_to_dev(&stream, &sinks);
    let output_buf = alloc_dev(&stream, batch_size * HIDDEN * 2);
    let float_workspace = alloc_dev(&stream, 128 * 1024 * 1024);
    let int_workspace = alloc_dev(&stream, 8 * 1024 * 1024);

    let library_path = std::ffi::CString::new(library_path).unwrap();
    let fa3 = crate::host::flashinfer::jit::ensure_compiled_fa3(HEAD_DIM, false);
    let init_status = unsafe { (fa3.external_init)(library_path.as_ptr()) };
    assert_eq!(init_status, 0, "external FA3 initialization failed");
    let run_status = unsafe {
        (fa3.external_sink_decode_run)(
            float_workspace.device_ptr(&stream).0 as *mut std::ffi::c_void,
            float_workspace.len(),
            int_workspace.device_ptr(&stream).0 as *mut std::ffi::c_void,
            int_workspace.len(),
            q_buf.device_ptr(&stream).0 as *const std::ffi::c_void,
            k_buf.device_ptr(&stream).0 as *const std::ffi::c_void,
            v_buf.device_ptr(&stream).0 as *const std::ffi::c_void,
            token_indices_buf.device_ptr(&stream).0 as *const i32,
            qo_indptr_buf.device_ptr(&stream).0 as *const i32,
            kv_indptr_buf.device_ptr(&stream).0 as *const i32,
            sinks_buf.device_ptr(&stream).0 as *const f32,
            output_buf.device_ptr(&stream).0 as *mut std::ffi::c_void,
            batch_size as i32,
            N_HEADS as i32,
            N_KV_HEADS as i32,
    ...
    assert_eq!(run_status, 0, "external FA3 decode failed");
    stream.synchronize().unwrap();

    let mut output_bytes = vec![0u8; batch_size * HIDDEN * 2];
    unsafe {
        cudarc::driver::result::memcpy_dtoh_async(
            &mut output_bytes,
            output_buf.device_ptr(&stream).0,
            stream.cu_stream(),
        )
        .unwrap();
    }
    stream.synchronize().unwrap();
    let result = bf16_bytes_to_f32s(&output_bytes);
