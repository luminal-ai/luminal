...
// Specialized uniform decode (one query row per request). Unlike the FA3
// prefill entry point this consumes the live device kv_indptr directly and
// needs no host planner or integer workspace. Q is the graph's heads-major
// [num_qo_heads, batch, head_dim] layout. Output can be either graph-native
// heads-major or projection-native token-major BF16.
...
// (s, heads, dim) bf16 → (heads, s, dim) bf16, retaining native storage.
int flashinfer_fa3_transpose_output_bf16(
    const void* src, void* dst,
    int batch, int heads, int dim,
    cudaStream_t stream);
