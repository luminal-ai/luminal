...
struct Args {
    ...
    /// Rank compiler candidates using measured device execution time.
...
fn main() -> Result<()> {
    let args = Args::parse();
    ensure!(cfg!(feature = "device"), "requires --features device");
    ...
    if let Some(path) = &args.plan_report {
        std::fs::write(
            ...
            serde_json::to_string_pretty(&json!({
                "model": format!("{:?}", args.model), "profile_on_device": args.profile,
    ...
    let report = json!({
        ...
        "profile_on_device": args.profile, "selected_counts": selected_counts,
