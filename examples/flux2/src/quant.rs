...
mod tests {
    ...
    /// End-to-end CUDA round-trip: construct a tiny NVFP4-quantized weight,
    /// dequantize through the HLIR pattern in [`Nvfp4Linear::dequant`], and
    /// compare element-wise against the pure-Rust reference. Validates:
    ///   * the F4E2M1 → F32 cast actually unpacks 4-bit packed bytes,
    ///   * the F8E4M3 → F32 cast produces the right numerical values,
    ///   * the (out, in/16) → (out, in) per-block broadcast lines up,
    ///   * the (1,) outer scale broadcasts to (out, in).
    #[test]
    fn cuda_dequant_matches_reference() {
        use super::{Nvfp4Linear, NVFP4_BLOCK};
        use luminal::prelude::*;
        use luminal_cuda_lite::{cudarc::driver::CudaContext, runtime::CudaRuntime};

        // 4 output rows × 32 input columns => 32 / 16 = 2 blocks per row.
        const OUT: usize = 4;
        const IN: usize = 32;
        const N_BLOCKS: usize = IN / NVFP4_BLOCK;

        // Build deterministic test data.
        // Packed FP4: OUT * IN / 2 = 64 bytes. Walk every byte from 0..64
        // (mod 256) so we exercise a broad range of 4-bit codes.
        let packed: Vec<u8> = (0..(OUT * IN / 2) as u8).collect();
        // Per-block FP8 scales: pick a few non-trivial bit patterns from the
        ...
        // Reference dequant in pure Rust.
        let mut expected = vec![0f32; OUT * IN];
        for o in 0..OUT {
            for blk in 0..N_BLOCKS {
                let s8 = fp8_bytes[o * N_BLOCKS + blk];
                let block_scale = fp8_e4m3_to_f32(s8);
                for k in 0..NVFP4_BLOCK {
                    ...
                    expected[flat] = FP4_E2M1_LUT[nibble] * block_scale * scale_2_value;
                }
            }
        }

        // HLIR graph: declare the three quant inputs, dequant to F32, mark
        // the result as an output.
        let mut cx = Graph::default();
        let layer = Nvfp4Linear::new("test", OUT, IN, &mut cx);
        let dequant = layer.dequant(DType::F32).output();
        ...
        // Upload raw packed bytes directly (skipping safetensors), exercising
        // the same code paths the loader fix uses.
        runtime.set_data(layer.weight, packed.clone());
        runtime.set_data(layer.weight_scale, fp8_bytes.clone());
        runtime.set_data(layer.weight_scale_2, vec![scale_2_value]);

        runtime = cx.search(runtime, 1);
        runtime.execute(&cx.dyn_map);

        let got = runtime.get_f32(dequant);
        ...
        for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
            ...
            assert!(
                d < 1e-5,
                "mismatch at idx {i}: got {g}, expected {e} (max_abs={max_abs})",
            );
        }
    }
}
